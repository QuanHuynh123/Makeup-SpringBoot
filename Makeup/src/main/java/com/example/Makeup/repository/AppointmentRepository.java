...
public interface AppointmentRepository extends JpaRepository<Appointment, UUID> {
  ...
  @Query(
      "SELECT new com.example.Makeup.dto.response.AppointmentsAdminResponse("
  ...
  List<AppointmentsAdminResponse> findAppointmentsByMonth(
      @Param("month") int month, @Param("year") int year, @Param("staffID") UUID staffID);
  ...
  @Query(
      "SELECT MONTH(a.makeupDate) AS month, COUNT(a) AS count "
          + "FROM Appointment a "
  ...
  @Query(
      "SELECT DATE_FORMAT(a.makeupDate, '%Y-%m-%d'), COUNT(a) "
          + "FROM Appointment a "
          + "WHERE a.makeupDate BETWEEN :startDate AND :endDate "
          + "GROUP BY DATE(a.makeupDate) "
          + "ORDER BY DATE(a.makeupDate) ASC")
  List<Object[]> findAppointmentsCountByDateRange(
      @Param("startDate") Date startDate, @Param("endDate") Date endDate);

  @Query(
      "SELECT DAY(a.makeupDate), COUNT(a) "
          + "FROM Appointment a "
  ...
  List<Object[]> findAppointmentsCountByCurrentMonth(
      @Param("currentMonth") int currentMonth, @Param("currentYear") int currentYear);

  @Query(
      "SELECT new com.example.Makeup.dto.model.AppointmentDTO("
