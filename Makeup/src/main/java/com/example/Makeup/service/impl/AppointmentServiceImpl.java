...
public class AppointmentServiceImpl implements IAppointmentService {
  ...
  public ApiResponse<AppointmentDTO> updateAppointment(
    ...
    TypeMakeup typeMakeup =
        typeMakeupRepository
            .findById(appointmentDTO.getTypeMakeupId())
            .orElseThrow(() -> new AppException(ErrorCode.COMMON_RESOURCE_NOT_FOUND));
    ...
    Staff staff =
        staffRepository
            .findById(appointmentDTO.getStaffId())
  ...
  public ApiResponse<AppointmentDTO> createAppointment(AppointmentRequest newAppointment) {
    ...
    Staff staff =
        staffRepository
            .findById(newAppointment.getStaffId())
            .orElseThrow(() -> new AppException(ErrorCode.STAFF_NOT_FOUND));
    appointment.setStaff(staff);

    TypeMakeup typeMakeup =
        typeMakeupRepository
            .findById(newAppointment.getTypeMakeupId())
            .orElseThrow(() -> new AppException(ErrorCode.COMMON_RESOURCE_NOT_FOUND));
    appointment.setTypeMakeup(typeMakeup);
  ...
  private List<WeekAppointmentsDTO> groupAppointmentsByWeek(
      List<AppointmentsAdminResponse> appointments, int month, int year) {
    ...
    for (int week = 1; week <= totalWeeks; week++) {
      List<AppointmentsAdminResponse> weekAppointments =
          weeklyAppointments.getOrDefault(week, new ArrayList<>());
      Date startDate = getWeekStartDate(week, month, year);
      Date endDate = getWeekEndDate(week, month, year);

      weekAppointmentsDTOList.add(
          new WeekAppointmentsDTO(week, startDate, endDate, weekAppointments));
    }
