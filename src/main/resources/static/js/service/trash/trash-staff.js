export const StaffModule = {
    ...
    loadPagedStaff() {
        ...
        fetch(`/api/admin/staffs?page=${this.currentPage}&size=${this.pageSize}&sort=${sortStr}${roleParam}`)
            .then(res => {
            ...
            .then(data => {
                if (data && data.result && data.result.content) {
                    this.staffList = data.result.content;
    ...
    searchStaff() {
        ...
        if (!this.staffList.length) {
            this.loadPagedStaff().then(() => this.searchStaff());
            return;
        }
